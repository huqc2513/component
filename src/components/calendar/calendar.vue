...
export default {
  ...
  methods: {
    ...
    comparisonList() {
      ...
      let ret = [];
      let startTimeMS =
        this.startTime && convertDateFromString(this.startTime).getTime();
      let endTimeMs =
        this.endTime && convertDateFromString(this.endTime).getTime();
    ...
    multipleChoose(item, index, idx) {
      ...
      //选择开始和结束时间
        ...
        if (
          new Date(this.startTime).getMonth() != new Date(item.time).getMonth()
        ) {
          toast("暂不支持跨月选择");
          return;
        }
        ...
        //反选逻辑  isNegation
        if (
          convertDateFromString(item.time) <
          convertDateFromString(this.startTime)
        ) {
